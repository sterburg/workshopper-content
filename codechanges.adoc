...
then click *Browse* and then *Pipelines*. Click the `nationalparks-pipeline` and
...
then click *Browse* and then *Builds*. Click the `nationalparks` build.
...
the `oc rollout undo --to-revision=X` and `oc rollout latest` commands you can move back- or forward between
...
[source]
----
oc rollout undo dc/nationalparks
----
...
[source]
----
deploymentconfig "nationalparks" rolled back
----
