...
You need to specify the SpringBoot application what is the location of this file, 
via a `JAVA_ARGS` environment variable with a variable `--spring.config.location`
pinting to the file.
...
oc env dc/nationalparks MONGODB_USER- MONGODB_PASSWORD- MONGODB_DATABASE- MONGODB_SERVER_HOST- \
       JAVA_ARGS=--spring.config.location=file:/opt/openshift/config/application.properties
----
